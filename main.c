...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  //if ( ABS(HAL_GetTick() - time) > 2000){
		  ...
		  HAL_UART_Transmit(&huart2, waterLevel, 1, 100);
		  HAL_UART_Transmit(&huart1, waterLevel, 1, 100);
		  //turn led if water level = 2
		  if (waterLevel[0] == '2'){
	  ...
	  if (pumpState == '1') {
		  //pwm pin set the motor to full speed
		  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET);
		  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_SET);
		  //for correct direction of motor
